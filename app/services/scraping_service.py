...
async def _fetch_new_ai_mode(
    ...
    try:
        ...
        # 2. Extract the actual job_id generated by the scraper
        data = response.json()
        job_id = data.get("job_id")
        if not job_id:
            logger.error("[Scraper:new_ai_mode] Scraper response missing job_id: %s", data)
            return {"success": False, "error": "No job_id returned by scraper"}

        logger.info(
            "[Scraper:new_ai_mode] Job accepted. job_id=%s  Waiting for webhook...",
            job_id,
        )

        # 3. Register the job in our tracker using the scraper's ID
...
async def _fetch_chatgpt(
    ...
    try:
        ...
        job_id = data.get("job_id")
        if not job_id:
            logger.error("[Scraper:chatgpt] Scraper response missing job_id: %s", data)
            return {"success": False, "error": "No job_id returned by scraper"}

        logger.info(
            "[Scraper:chatgpt] Job accepted. job_id=%s  Waiting for webhook...",
            job_id,
        )
